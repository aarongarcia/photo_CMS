<?php
	...
	class contentExtensionPageTemplatesEdit extends contentBlueprintsPages {
		...
		function action(){
			...
			if(@array_key_exists('delete', $_POST['action'])) {
			    ...
			    $page = $this->_Parent->Database->fetchRow(0, "SELECT * FROM tbl_pages_templates WHERE `id` = '$page_id'");

				$filename = Lang::createHandle($_POST['fields']['title']);

				$this->_Parent->Database->delete('tbl_pages_templates', " `id` = '$page_id'");
				$this->_Parent->Database->delete('tbl_pages_types', " `page_id` = '$page_id'");	  
				$this->_Parent->Database->query("UPDATE tbl_pages_templates SET `sortorder` = (`sortorder` + 1) WHERE `sortorder` < '$page_id'");

				General::deleteFile(PAGES . "/templates/" . $filename . ".xsl");

				redirect(URL . '/symphony/extension/pagetemplates/manage');
			...
			elseif(@array_key_exists('save', $_POST['action'])){
				...
				if(empty($this->_errors)){
					...
					$new_filename = Lang::createHandle($fields['title']);
					...
					$current = $this->_Parent->Database->fetchRow(0, "SELECT * FROM `tbl_pages_templates` WHERE `id` = '$page_id' LIMIT 1");	

					$current_filename = Lang::createHandle($current['title']);
					...
					if($this->_Parent->Database->fetchRow(0, "SELECT * FROM `tbl_pages_templates` 
						...
						if(!$write = General::writeFile(PAGES . "/templates/$new_filename.xsl" , $fields['body'], $this->_Parent->Configuration->get('write_mode', 'file')))
						...
						## Write Successful, add record to the database
							...
							if($new_filename != $current_filename) @unlink(PAGES . "/templates/$current_filename.xsl");
							
							## No longer need the body text
							unset($fields['body']);

							## Insert the new data
							if(!$this->_Parent->Database->update($fields, 'tbl_pages_templates', "`id` = '$page_id'")) $this->pageAlert(__('Unknown errors occurred while attempting to save. Please check your <a href="%s">activity log</a>.', array(URL.'/symphony/system/log/')), Alert::ERROR);
			                    ...
			                    redirect(URL . "/symphony/extension/pagetemplates/edit/$page_id/saved/");
